...
export default class JsBandwidth {
    ...
    testDownloadSpeed(options) {
        ...
        var r1 = r.then(
            function(response) {
                return {
                    downloadSpeed: JsBandwidth.calculateBandwidth((response.data || response).length, start),
    ...
    testUploadSpeed(options) {
        ...
        var r1 = r.then(
            function(response) {
                return {
                    uploadSpeed: JsBandwidth.calculateBandwidth(options.uploadData.length, start)
                };
            });
        r1.cancel = r.cancel;
        return r1;
    ...
    testSaveData() {
        ...
        if (navi.connection && navi.connection.saveData) {
            r1 = navi.connection.saveData;
        }
                        ...
                        var r1 = r.then(
                            function(uploadResult) {
                                return {
                                    ...
                                    saveDataMode: self.testSaveData()
                                };
                            }
                        );
                        r1.cancel = r.cancel;
